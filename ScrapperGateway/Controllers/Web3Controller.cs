...
namespace ScrapperGateway.Controllers
{
    ...
    public class Web3Controller : ControllerBase
    {
        ...
        public async Task<IActionResult> GetCarListings(string keywords, int pagestoscrap, int? category, string? latitude, string? longitude, int? minprice, int? maxprice, bool shippingAviable, bool isProgrammed)
        {
            ...
            var jsonResponse = await _web3Service.GetWallapop(keywords, pagestoscrap, category,  latitude, longitude, minprice, maxprice,shippingAviable, isProgrammed);
